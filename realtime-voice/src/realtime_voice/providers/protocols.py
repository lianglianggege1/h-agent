...
class Message:
    ...
    def _read_event(self, buffer: io.BytesIO) -> None:
        ...
        if event_bytes:
            ...
            try:
                ...
                # For undefined event types, just store the value
    ...
    def __str__(self) -> str:
        ...
        event_str = (
            str(self.event)
            if isinstance(self.event, EventType)
            else f"EventType({self.event})"
        )
        ids = ""
        if self.session_id:
...
async def full_client_request(websocket: Any, payload: bytes) -> None:
...
async def audio_only_client(
    websocket: Any, payload: bytes, flag: MsgTypeFlagBits
) -> None:
...
async def cancel_session(websocket: Any, session_id: str) -> None:
    """Cancel session"""
    msg = Message(type=MsgType.FullClientRequest, flag=MsgTypeFlagBits.WithEvent)
