...
class HarnessAgentExecutorTest {
    ...
    @Test
    void shouldStreamOnlyParentTextToLegacyChunkAndPersistAuthoritativeParentResult() {
        ...
        when(harnessAgent.streamEvents(eq("solve it"), any(RuntimeContext.class)))
                .thenReturn(Flux.just(parentDelta, childDelta, new AgentResultEvent(finalReply)));
        ...
        assertEquals("parent live", events.get(1).content());
        assertEquals(0, events.stream().filter(event -> "child private".equals(event.content())).count());
        assertEquals(persisted, events.get(4).message());
        ...
        verify(chatSessionService).appendAssistantMessage(1L, "session-1", "parent final");
